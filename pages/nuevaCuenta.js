...
const NuevaCuenta = () => {
  ...
  const [boton, cargarBoton] = useState(false);
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      password: Yup.string()
        ...
        .min(6, "* El passord debe ser de al menos 6 caracteres"),
    }),
    onSubmit: async (valores) => {
      ...
      try {
        const { data } = await nuevoUsuario({
          variables: {
            // son lo mismo asi q puede ir sin el objeto o con el objeto
            input: {
              nombre: nombre,
              apellido,
              email: email,
        ...
        // Redirigiar al usuario para iniciar sesión
      } catch (error) {
        guardarMensaje(error.message.replace());
